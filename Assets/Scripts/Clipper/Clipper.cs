...
* - I renamed IntPoint to Vector2 (because it contains longs, not ints, and     *
...
* - I changed Vector2 from a "class" to a "struct". In fact, the code was       *
...
*   as much memory on x64 if Vector2 is a struct.)                              *
...
namespace ClipperLib
{
	...
	public class ClipperBase
	{
		...
		internal bool PointInPolygon(Vector2 pt, OutPt pp, bool UseFulllongRange)
		{
			OutPt pp2 = pp;
			bool result = false;
			if (UseFulllongRange) {
		...
		protected bool SlopesEqual(Vector2 pt1, Vector2 pt2,
		...
		public bool AddPolygon(Polygon pg, PolyType polyType)
		{
			...
			p.Add(new Vector2(pg[0].x, pg[0].y));
			int j = 0;
			for (int i = 1; i < len; ++i) {
				...
				if (j < p.Count)
					...
					p.Add(new Vector2(pg[i].x, pg[i].y));
			}
			if (j < 2)
				return false;
		...
		private void InitEdge(TEdge e, TEdge eNext,
		  TEdge ePrev, Vector2 pt, PolyType polyType)
		{
			e.next = eNext;
			e.prev = ePrev;
			e.xcurr = pt.x;
	...
	/// </summary>
	/// <remarks>
	/// This implementation expects integer input points (<see cref="Vector2"/>), 
	/// although it still uses floating-point math internally for some operations.
	/// <para/>
	/// Usage: use List{Vector2} to define each polygon, then call <see cref="AddPolygon"/>
	...
	public class Clipper : ClipperBase
	{
		...
		private bool ExecuteInternal(bool fixHoleLinkages)
		{
			...
			try {
				...
				if (m_CurrentLM == null)
					return true;
				float botY = PopScanbeam();
				do {
					InsertLocalMinimaIntoAEL(botY);
					m_HorizJoins.Clear();
					ProcessHorizontals();
					float topY = PopScanbeam();
		...
		private void AddJoin(TEdge e1, TEdge e2, int e1OutIdx, int e2OutIdx)
		{
			...
			jr.pt1a = new Vector2(e1.xcurr, e1.ycurr);
			jr.pt1b = new Vector2(e1.xtop, e1.ytop);
			if (e2OutIdx >= 0)
				jr.poly2Idx = e2OutIdx;
			else
				jr.poly2Idx = e2.outIdx;
			jr.pt2a = new Vector2(e2.xcurr, e2.ycurr);
			jr.pt2b = new Vector2(e2.xtop, e2.ytop);
			m_Joins.Add(jr);
		}
		//------------------------------------------------------------------------------
		...
		private void InsertLocalMinimaIntoAEL(float botY)
		{
			while (m_CurrentLM != null && (m_CurrentLM.y == botY))
			{
				...
				if (IsContributing(lb))
					AddLocalMinPoly(lb, rb, new Vector2(lb.xcurr, m_CurrentLM.y));

				//if any output polygons share an edge, they'll need joining later ...
				if (rb.outIdx >= 0) {
					if (rb.dx == Horizontal) {
						for (int i = 0; i < m_HorizJoins.Count; i++) {
							Vector2 pt = new Vector2(), pt2 = new Vector2(); //used as dummy params.
							HorzJoinRec hj = m_HorizJoins[i];
							//if horizontals rb and hj.edge overlap, flag for joining later ...
							if (GetOverlapSegment(new Vector2(hj.edge.xbot, hj.edge.ybot),
								new Vector2(hj.edge.xtop, hj.edge.ytop),
								new Vector2(rb.xbot, rb.ybot),
								new Vector2(rb.xtop, rb.ytop),
								ref pt, ref pt2))
				...
				if (lb.nextInAEL != rb) {
					...
					Vector2 pt = new Vector2(lb.xcurr, lb.ycurr);
					while (e != rb) {
						if (e == null)
		...
		private void AddLocalMinPoly(TEdge e1, TEdge e2, Vector2 pt)
		{
			TEdge e, prevE;
		...
		private void AddOutPt(TEdge e, TEdge altE, Vector2 pt)
		{
		...
		private bool GetOverlapSegment(Vector2 pt1a, Vector2 pt1b, Vector2 pt2a,
			Vector2 pt2b, ref Vector2 pt1, ref Vector2 pt2)
		{
		...
		private bool FindSegment(ref OutPt pp, ref Vector2 pt1, ref Vector2 pt2)
		{
			if (pp == null)
				return false;
			OutPt pp2 = pp;
			Vector2 pt1a = new Vector2(pt1.x,pt1.y);
			Vector2 pt2a = new Vector2(pt2.x, pt2.y);
			do {
		...
		internal bool Pt3IsBetweenPt1AndPt2(Vector2 pt1, Vector2 pt2, Vector2 pt3)
		{
			if (pt1 == pt3 || pt2 == pt3)
				return true;
			else if (pt1.x != pt2.x)
		...
		private OutPt InsertPolyPtBetween(OutPt p1, OutPt p2, Vector2 pt)
		{
		...
		private float GetDx(Vector2 pt1, Vector2 pt2)
		{
			if (pt1.y == pt2.y)
				return Horizontal;
			else
		...
		private OutRec GetLowermostRec(OutRec outRec1, OutRec outRec2)
		{
			...
			if (bPt1.pt.y > bPt2.pt.y)
				...
				float y1 = Math.Max(outRec1.bottomE1.ybot, outRec1.bottomE2.ybot);
				float y2 = Math.Max(outRec2.bottomE1.ybot, outRec2.bottomE2.ybot);
				if (y2 == y1 || (y1 > bPt1.pt.y && y2 > bPt1.pt.y)) {
					float dx1 = Math.Max(outRec1.bottomE1.dx, outRec1.bottomE2.dx);
					float dx2 = Math.Max(outRec2.bottomE1.dx, outRec2.bottomE2.dx);
					if (dx2 > dx1)
						return outRec2;
					else
						return outRec1;
		...
		private void DoEdge1(TEdge edge1, TEdge edge2, Vector2 pt)
		{
		...
		private void DoEdge2(TEdge edge1, TEdge edge2, Vector2 pt)
		{
		...
		private void DoBothEdges(TEdge edge1, TEdge edge2, Vector2 pt)
		{
			AddOutPt(edge1, edge2, pt);
			AddOutPt(edge2, edge1, pt);
			SwapSides(edge1, edge2);
		...
		private void IntersectEdges(TEdge e1, TEdge e2, Vector2 pt, Protects protects)
		{
		...
		private void ProcessHorizontal(TEdge horzEdge)
		{
				  ...
				  ((Direction == Direction.RightToLeft) && (e.xcurr >= horzLeft))) {
					...
					if (e == eMaxPair) {
						...
						if (Direction == Direction.LeftToRight)
							IntersectEdges(horzEdge, e, new Vector2(e.xcurr, horzEdge.ycurr), 0);
						else
							IntersectEdges(e, horzEdge, new Vector2(e.xcurr, horzEdge.ycurr), 0);
						if (eMaxPair.outIdx >= 0)
							throw new ClipperException("ProcessHorizontal error");
						return;
					} else if (e.dx == Horizontal && !IsMinima(e) && !(e.xcurr > e.xtop)) {
						if (Direction == Direction.LeftToRight)
							IntersectEdges(horzEdge, e, new Vector2(e.xcurr, horzEdge.ycurr),
							  (IsTopHorz(horzEdge, e.xcurr)) ? Protects.Left : Protects.Both);
						else
							IntersectEdges(e, horzEdge, new Vector2(e.xcurr, horzEdge.ycurr),
			...
			if (horzEdge.nextInLML != null) {
				if (horzEdge.outIdx >= 0)
					AddOutPt(horzEdge, null, new Vector2(horzEdge.xtop, horzEdge.ytop));
				UpdateEdgeIntoAEL(ref horzEdge);
			} else {
				if (horzEdge.outIdx >= 0)
					IntersectEdges(horzEdge, eMaxPair,
						new Vector2(horzEdge.xtop, horzEdge.ycurr), Protects.Both);
		...
		private bool IsTopHorz(TEdge horzEdge, float XPos)
		{
		...
		private bool ProcessIntersections(float botY, float topY)
		{
			if (m_ActiveEdges == null)
				return true;
			try {
		...
		private void BuildIntersectList(float botY, float topY)
		{
			if (m_ActiveEdges == null)
				return;
		...
		private float TopX(Vector2 pt1, Vector2 pt2, float currentY)
		{
			...
			if (currentY >= pt1.y)
				...
				return (float)Round(pt1.x + (currentY - pt1.y) * q);
			}
		}
		//------------------------------------------------------------------------------

		private void AddIntersectNode(TEdge e1, TEdge e2, Vector2 pt)
		{
		...
		private bool IntersectPoint(TEdge edge1, TEdge edge2, ref Vector2 ip)
		{
			float b1, b2;
			...
			while (e != null) {
				...
				if (IsMaxima(e, topY) && GetMaximaPair(e).dx != Horizontal) {
					...
					if (IsIntermediate(e, topY) && e.nextInLML.dx == Horizontal) {
						if (e.outIdx >= 0) {
							AddOutPt(e, null, new Vector2(e.xtop, e.ytop));

							for (int i = 0; i < m_HorizJoins.Count; ++i) {
								Vector2 pt = new Vector2(), pt2 = new Vector2();
								HorzJoinRec hj = m_HorizJoins[i];
								if (GetOverlapSegment(new Vector2(hj.edge.xbot, hj.edge.ybot),
									new Vector2(hj.edge.xtop, hj.edge.ytop),
									new Vector2(e.nextInLML.xbot, e.nextInLML.ybot),
									new Vector2(e.nextInLML.xtop, e.nextInLML.ytop), ref pt, ref pt2))
			...
			while (e != null) {
				if (IsIntermediate(e, topY)) {
					...
					if (e.outIdx >= 0 && e.prevInAEL != null && e.prevInAEL.outIdx >= 0 &&
						...
						SlopesEqual(new Vector2(e.xbot, e.ybot), new Vector2(e.xtop, e.ytop),
						new Vector2(e.xbot, e.ybot),
						new Vector2(e.prevInAEL.xtop, e.prevInAEL.ytop), m_UseFullRange)) {
						AddOutPt(e.prevInAEL, null, new Vector2(e.xbot, e.ybot));
						AddJoin(e, e.prevInAEL, -1, -1);
						  ...
						  SlopesEqual(new Vector2(e.xbot, e.ybot), new Vector2(e.xtop, e.ytop),
							new Vector2(e.xbot, e.ybot),
							new Vector2(e.nextInAEL.xtop, e.nextInAEL.ytop), m_UseFullRange)) {
						AddOutPt(e.nextInAEL, null, new Vector2(e.xbot, e.ybot));
		...
		private void DoMaxima(TEdge e, float topY)
		{
			...
			while (eNext != eMaxPair) {
				...
				IntersectEdges(e, eNext, new Vector2(X, topY), Protects.Both);
			...
			if (e.outIdx < 0 && eMaxPair.outIdx < 0) {
				...
				IntersectEdges(e, eMaxPair, new Vector2(X, topY), Protects.None);
			} else
				throw new ClipperException("DoMaxima error");
		}
		//------------------------------------------------------------------------------
		...
		private bool Orientation(OutRec outRec, bool UseFull64BitRange)
		{
			...
			Vector2 vec1 = new Vector2(op.pt.x - opPrev.pt.x, op.pt.y - opPrev.pt.y);
			Vector2 vec2 = new Vector2(opNext.pt.x - op.pt.x, opNext.pt.y - op.pt.y);
		...
		private void JoinCommonEdges(bool fixHoleLinkages)
		{
			for (int i = 0; i < m_Joins.Count; i++) {
				...
				Vector2 pt1 = new Vector2(j.pt2a.x, j.pt2a.y);
				Vector2 pt2 = new Vector2(j.pt2b.x, j.pt2b.y);
				Vector2 pt3 = new Vector2(j.pt1a.x, j.pt1a.y);
				Vector2 pt4 = new Vector2(j.pt1b.x, j.pt1b.y);
